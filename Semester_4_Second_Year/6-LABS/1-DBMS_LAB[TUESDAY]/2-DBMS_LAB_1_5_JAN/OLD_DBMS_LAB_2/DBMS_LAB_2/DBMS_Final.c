...
void List_Range()
{
    ...
    printf("2 -> List all patients whose Adm Date between given date till 10 days next\n");
    printf("3 -> List all patients whose Adm month between given month till 5 months next\n");
    int ch;
    printf("\nChoice [1/2/3] : ");
    scanf("%d", &ch);
    ...
    case 2:
        ...
        //m IS MAXIMUM DAYS OF A SPECIFIC MONTH
        int i = 1, m;
        ...
        while (fread(&p, sizeof(struct patient), 1, fp))
        {
            ...
            while (i < 11)
            {
                if (cmp(p.admission, d[i]) == 0) //cmp() RETURNS 0 IF EQUAL
                {
                    printf("\n~~ PATIENT DETAILS ~~\n");
                    printf("\nPatient Number : %d", p.pno);
                    printf("\nFirst Name     : %s", p.first_name);
